...
@RestController
@FeignClient(value = "login", url = "${NeteaseCloudMusicApi.url}")
public interface LoginApi {

    @GetMapping("/login/qr/create")
    String qrCreate(@RequestParam("key") String key, @RequestParam("qrimg") Boolean qrImg);

    @GetMapping("/login/qr/key")
    String qrKey();

    @GetMapping("/login/qr/check")
    String qrCheck(@RequestParam(value = "key", required = false) String key);
