<template>
  <div>
    ...
    <div class="myCanvas">
      <canvas id="myCanvas" style="width: 600px; height: 200px">
      </canvas>
    </div>
  </div>
</template>
...
export default class MusicPlayer extends Vue {
  ...
  circleLeft: string | null = null
  barWidth: string | null = null
  ...
  tracks: Array<Track> = [
    {
      ...
      artist: "test",
      cover: "",
      source: require('../../assets/music/test.mp3'),
      url: "",
      favorited: false
    }
  ]
  ...
  onPlay() {
    ...
    const source = audCtx.createMediaElementSource(this.audio); //创建音频源节点
    this.analyser.fftSize = 64; //一个无符号长整形 (unsigned long) 的值，代表了快速傅里叶变换(分析器)的窗口大小
    //创建数组，用于接受节点分析器分析的数据
  ...
  draw() {
    const cvs = this.$el.querySelector('#myCanvas') as HTMLCanvasElement;
  ...
  initCancas() {
    const canvas = this.$el.querySelector('#myCanvas') as HTMLCanvasElement;
  ...
  generateTime() {
    ...
    let dursec = Math.floor(this.audio.duration - parseInt(durmin) * 60) + "";
    let curmin = Math.floor(this.audio.currentTime / 60) + "";
    let cursec = Math.floor(this.audio.currentTime - parseInt(curmin) * 60) + "";
  ...
  updateBar(x: number) {
    ...
    this.audio.currentTime = Math.floor((maxduration * percentage) / 100);
    this.audio.play();
  }
